...
namespace com.etsoo.EasyPdf
{
    ...
    public class PdfParser
    {
        public static async Task<PdfParser> ParseAsync(Stream pdfStream)
        {
            ...
            var version = PdfVersion.Parse(reader.ReadLine());
            if (version is null)
            {
                throw new InvalidDataException("No version line");
            }
            parser.Version = version.Version;

            // Second line is binary check
            var binaryCheck = PdfBinaryCheck.Parse(reader.ReadLine());
            if (binaryCheck is null)
            {
                throw new InvalidDataException("No binary check line");
            }

            // Body start
            var pos = reader.CurrentPosition;

            // Depending on the stream's "CanSeek" feature
            if (!pdfStream.CanSeek)
            {
                ...
                // eof
                var eof = PdfEOF.Parse(lastLine.Span);
                if (eof == null)
                {
                    throw new InvalidDataException("No End of File (EOF) line");
                }
            ...
            else
            {
                ...
                {
                    var line = await reader.ReadLineAsync(PureStreamReadWay.LineFeedOnly);

                    // 1 0 obj
                    var obj = await PdfObjectParser.ParseAsync(line, reader);
                    if (obj is not null)
                    {
                        continue;
                    }
                    ...
                    // xref
                    if (line.Span.SequenceEqual(PdfConstants.XrefBytes))
                    {
                        // First trailer
                        await parser.ParseTrailerAsync(reader, false);
                    }
                }
            }
